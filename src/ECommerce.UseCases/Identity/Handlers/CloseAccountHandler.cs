...
public sealed class CloseAccountHandler(
    ...
    IAuditLogWriter auditLogWriter) : IRequestHandler<CloseAccountCommand, Result>
{
    public async Task<Result> Handle(CloseAccountCommand request, CancellationToken cancellationToken)
    {
        ...
        await auditLogWriter.WriteAsync(new AuditOperation(
            AuditActions.AccountClosed,
            "Customer",
            customer.Id.ToString(),
            After: new { customerId = customer.Id, email = customer.Email },
            ActorId: customer.Id,
