...
public sealed class OrderShippedConsumerTests
{
    private static OrderShipped CreateOrderShipped() =>
        new(Guid.NewGuid(), "E-20260807-000001", "ahmed@example.com", "aramex", ["1Z999AA10123456784"]);
        ...
        CapturingOrderNotifier notifier)
    {
        ...
        services.AddMassTransitTestHarness(config => config.AddConsumer<OrderShippedConsumer>());
    ...
    public async Task Consume_FirstDelivery_Claims_Inbox_And_Notifies()
    {
        ...
        try
        {
            ...
            await harness.Bus.Publish(CreateOrderShipped(), context => context.MessageId = messageId);

            Assert.True(await harness.Consumed.Any<OrderShipped>());
            Assert.Single(notifier.Shipped);
    ...
    public async Task Consume_Duplicate_Message_Id_Skips_Notifier()
    {
        ...
        {
            var messageId = Guid.NewGuid();
            var orderShipped = CreateOrderShipped();

            await harness.Bus.Publish(orderShipped, context => context.MessageId = messageId);
            Assert.True(await harness.Consumed.Any<OrderShipped>());
            await harness.Bus.Publish(orderShipped, context => context.MessageId = messageId);
    ...
    public async Task Consume_Message_Without_Id_Processes_Without_Dedupe()
    {
        ...
        try
        {
            await harness.Bus.Publish(CreateOrderShipped(), context => context.MessageId = Guid.Empty);
