...
| Aggregate | Detail |
|-----------|--------|
| **FulfillmentTask** | Root. Attributes: Id, OrderId, WarehouseId, ParentTaskId (null = top-level), Status (Queued/Assigned/Picking/Packed/Shipped/Cancelled), AssignedPickers, Priority, Zone. Methods: `Create(orderId, warehouseId, priority, utcNow, zone?)`, `Assign(pickerId, utcNow)` (Queued→Assigned), `StartPicking(utcNow)` (Assigned→Picking), `MarkPacked(utcNow)` (Picking→Packed), `MarkShipped(utcNow)` (Packed→Shipped), `Cancel(utcNow)` (non-terminal states), `Split(warehouseId, itemIds, priority, zone, utcNow)` (Queued only; moves a proper subset of items to a new child task with `ParentTaskId = Id`, Version++, event `FulfillmentTaskSplit`; move-all or move-none returns `ERR_FLM_011 InvalidSplit`, non-Queued returns `NotQueued`). Events: `FulfillmentTaskCreated`, `FulfillmentTaskAssigned`, `FulfillmentTaskPicking`, `FulfillmentTaskPacked`, `FulfillmentTaskShipped`, `FulfillmentTaskCancelled`, `FulfillmentTaskSplit`. |
...
| **FulfillmentTaskItem** | Child of FulfillmentTask. Attributes: TaskId, ProductId, Sku, Quantity, Bin. Added via `FulfillmentTask.AddItem(productId, sku, quantity, bin)`; moved between tasks via `MoveTo(taskId)` during a split. |
...
Order status integration: `StartFulfillment` (AwaitingFulfillment→Picking), `MarkPacked` (Picking→Packed), `Ship` (Packed→Shipped), `Deliver` (Shipped→Delivered) — driven by the fulfillment task/shipment handlers. With split tasks, handlers are guard-conditioned so the order only advances when the whole set is ready: picking starts on the first task (`AwaitingFulfillment` guard), packing on the first packed task (`Picking` guard), `Ship` runs only when **all** tasks are Shipped/Cancelled (`HasUnshippedTasksAsync`), and `Deliver` runs only when **all** shipments are Delivered (`HasUndeliveredShipmentsAsync`). Shipping address correction (BR-1507, US-H-006): `Order.UpdateShippingAddress(newAddress, ...)` rejects Shipped/Delivered/Completed/Cancelled with `ERR_ORD_006 AddressCorrectionNotAllowed`, is a no-op on identical addresses, and raises `OrderShippingAddressUpdated(PreviousAddress, NewAddress)` for audit.
